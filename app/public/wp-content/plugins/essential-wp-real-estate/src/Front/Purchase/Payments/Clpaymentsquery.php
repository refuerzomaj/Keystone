 ...
 * @since 1.8
 */
class Clpaymentsquery extends Clpaymentstats {


	/**
	 * The args to pass to the cl_get_payments() query
	 *
	 * @var array
	 * @since 1.8
	 */
	...
	public function __construct( $args = array() ) {
		$defaults = array(
			...
			'user'            => null,
			'customer'        => null,
			'status'          => cl_get_payment_status_keys(),
			'meta_key'        => null,
			'year'            => null,
			'month'           => null,
			'day'             => null,
	...
	public function get_payments() {
		...
		do_action( 'cl_pre_get_payments', $this );
		...
		if ( $query->have_posts() ) {
			...
			while ( $query->have_posts() ) {
				...
				$this->payments[] = apply_filters( 'cl_payment', $payment, $payment_id, $this );
			}
		}

		add_action( 'cl_post_get_payments', array( $this, 'date_filter_post' ) );

		do_action( 'cl_post_get_payments', $this );
	...
	public function orderby() {
		switch ( $this->args['orderby'] ) {
			case 'amount':
				...
				$this->__set( 'meta_key', '_cl_payment_total' );
				break;
			default:
	...
	public function user() {
		...
		if ( is_numeric( $this->args['user'] ) ) {
			$user_key = '_cl_payment_user_id';
		} else {
			$user_key = '_cl_payment_user_email';
		}
	...
	public function search() {
		...
		if ( ! empty( $this->args['search_in_notes'] ) ) {
			...
			$notes = cl_get_payment_notes( 0, $search );
			...
			$key         = $is_email ? '_cl_payment_user_email' : '_cl_payment_purchase_key';
			$search_meta = array(
				'key'     => $key,
				'value'   => $search,
				'compare' => 'LIKE',
			...
			cl_admin_get_option( 'enable_sequential' ) &&
			( false !== strpos( $search, cl_admin_get_option( 'sequential_prefix' ) ) ||
				false !== strpos( $search, cl_admin_get_option( 'sequential_postfix' ) ) )
		) {
			...
			if ( is_object( $post ) && $post->post_type == 'cl_payment' ) {
	...
	public function listing() {
		...
		if ( is_array( $this->args['listing'] ) ) {
			unset( $args['post_parent'] );
			$args['post_parent__in'] = $this->args['listing'];
		}

		$sales = $cl_logs->get_connected_logs( $args );
		...
		if ( ! empty( $sales ) ) {
			...
			foreach ( $sales as $sale ) {
				$payments[] = get_post_meta( $sale, '_cl_log_payment_id', true );
			}
