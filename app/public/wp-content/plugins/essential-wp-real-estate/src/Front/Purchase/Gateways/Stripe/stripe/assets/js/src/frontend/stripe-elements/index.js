...
const DEFAULT_ELEMENTS = {
	'card': '#cl-stripe-card-element',
}

const DEFAULT_SPLIT_ELEMENTS = {
	'cardNumber': '#cl-stripe-card-element',
	'cardExpiry': '#cl-stripe-card-exp-element',
	'cardCvc': '#cl-stripe-card-cvc-element',
}

let ELEMENTS_OPTIONS = { ...cl_stripe_vars.elementsOptions };

/**
 * Mounts Elements based on payment form configuration.
 *
 * Assigns a `cardElement` object to the global `clStripe` object
...
export function createPaymentForm( elementsInstance, elements ) {
	...
	if ( ! elements ) {
		elements = ( 'true' === cl_stripe_vars.elementsSplitFields )
	...
	window.clStripe.cardElement = mountedEl;
...
function generateElementStyles() {
	...
	const cardNameEl = document.querySelector( '.card-name.cl-input' );
	...
	if ( ! document.getElementById( 'cls-stripe-element-styles' ) ) {
		const styleTag = document.createElement( 'style' );

		styleTag.innerHTML = `
			.cl-stripe-card-element.StripeElement,
			.cl-stripe-card-exp-element.StripeElement,
			.cl-stripe-card-cvc-element.StripeElement {
		...
		styleTag.id = 'cls-stripe-element-styles';
 ...
 */
export function mountCardElement( elementsInstance, toMount = '#cl-stripe-card-element' ) {
	...
	const splitFields = document.getElementById( 'cl-card-details-wrap' );
...
function handleElementError( event, el ) {
	const newCardContainer = el.closest( '.cl-stripe-new-card' );
	const errorsContainer  = newCardContainer.querySelector( '#cl-stripe-card-errors' );
...
export function getPaymentMethod( billingDetailsForm, cardElement ) {
	const selectedPaymentMethod = $( 'input[name="cl_stripe_existing_card"]:checked' );
