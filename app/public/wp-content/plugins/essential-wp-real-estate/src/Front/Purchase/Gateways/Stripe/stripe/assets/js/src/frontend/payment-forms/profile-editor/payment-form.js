/* global cl_stripe_vars, location */
...
export function paymentForm() {
	...
	document.querySelector( '.cl-stripe-add-new' ).addEventListener( 'click', onToggleForm );
	document.getElementById( 'cl-stripe-add-new-cancel' ).addEventListener( 'click', onToggleForm );
	document.getElementById( 'cl-stripe-add-new-card' ).addEventListener( 'submit', onAddPaymentMethod );
...
function onToggleForm( e ) {
	...
	const form = document.getElementById( 'cl-stripe-add-new-card' );
	const formFields = form.querySelector( '.cl-stripe-add-new-card' );
	const isFormVisible = 'block' === formFields.style.display;

	const cancelButton = form.querySelector( '#cl-stripe-add-new-cancel' );
...
export function addPaymentMethod( paymentMethod ) {
	apiRequest( 'cls_add_payment_method', {
		payment_method_id: paymentMethod.id,
		nonce: document.getElementById( 'cl-stripe-add-card-nonce' ).value,
	} )
		/**
...
function disableForm() {
	const submit = document.querySelector( '.cl-stripe-add-new' );
...
export function handleNotice( error, type = 'error' ) {
	...
	const notice = generateNotice(
		( error && error.message ) ? error.message : cl_stripe_vars.generic_error,
		type
	);

	// Hide previous notices.
	forEach( document.querySelectorAll( '.cl-stripe-alert' ), function( alert ) {
		alert.remove();
	} );

	// Show new notice.
	document.querySelector( '.cl-stripe-add-card-actions' )
		.insertBefore( notice, document.querySelector( '.cl-stripe-add-new' ) );
}
