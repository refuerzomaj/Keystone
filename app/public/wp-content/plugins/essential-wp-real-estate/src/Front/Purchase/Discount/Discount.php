...
class Discount {
	...
	public function __get( $key ) {
		...
		if ( method_exists( $this, 'get_' . $key ) ) {
			...
			return new WP_Error( 'cl-discount-invalid-property', sprintf( __( 'Can\'t get property %s', 'essential-wp-real-estate' ), $key ) );
		}
	}
	...
	private function find_by_code( $code = '' ) {
		...
		$discounts = WPERECCP()->front->discountaction->cl_get_discounts(
			array(
				'meta_key'       => '_cl_discount_code',
				'meta_value'     => $code,
				'posts_per_page' => 1,
				'post_status'    => 'any',
				'fields'         => 'ids',
	...
	private function find_by_name( $name = '' ) {
		...
		$discounts = WPERECCP()->admin->discount_action->cl_get_discounts(
			array(
	...
	private function setup_discount( $discount = null ) {
		...
		if ( 'cl_discount' !== $discount->post_type ) {
			return false;
		}

		do_action( 'cl_pre_setup_discount', $this, $discount );
		...
		do_action( 'cl_setup_discount', $this, $discount );
	 ...
	 * @access private
	 *
	 * @return array listing requirements.
	 */
	...
	public function get_meta( $key = '', $single = true ) {
		$meta = get_post_meta( $this->ID, '_cl_discount_' . $key, $single );
		return $meta;
	}
	...
	public function update_meta( $key = '', $value = '', $prev_value = '' ) {
		...
		$key = '_cl_discount_' . $key;

		$value = apply_filters( 'cl_update_discount_meta_' . $key, $value, $this->ID );
	...
	public function get_code() {
		 ...
		 */
		return apply_filters( 'cl_get_discount_code', $this->code, $this->ID );
	}
	...
	public function get_status() {
		/**
		   ...
		   * @param int    $ID   Discount ID.
		   */
		return apply_filters( 'cl_get_discount_status', $this->status, $this->ID );
	}
	...
	public function get_status_label() {
		switch ( $this->status ) {
			case 'expired':
				$label = __( 'Expired', 'essential-wp-real-estate' );
				break;
			case 'inactive':
				$label = __( 'Inactive', 'essential-wp-real-estate' );
				break;
			case 'active':
			default:
				$label = __( 'Active', 'essential-wp-real-estate' );
				break;
		}
		 ...
		 * @param int    $ID     Discount ID.
		 */
		return apply_filters( 'cl_get_discount_status_label', $label, $this->status, $this->ID );
	}
	...
	public function get_type() {
		 ...
		 * @param int    $ID   Discount ID.
		 */
		return apply_filters( 'cl_get_discount_type', $this->type, $this->ID );
	}
	...
	public function get_amount() {
		/**
		   ...
		   * @param int    $ID    Discount ID.
		   */
		return (float) apply_filters( 'cl_get_discount_amount', $this->amount, $this->ID );
	}
	 ...
	 * @since 2.7
	 *
	 * @return array IDs of required listings.
	 */
	public function get_product_reqs() {
		...
		/**
		 * Filters the listing requirements.
		 *
		 * @since 2.7
		 *
		 ...
		 * @param int   $ID           Discount ID.
		 */
		return (array) apply_filters( 'cl_get_discount_product_reqs', $this->product_reqs, $this->ID );
	}

	/**
	 * Retrieve the listings that are excluded from having this discount code applied.
	 *
	 * @since 2.7
	 *
	 * @return array IDs of excluded listings.
	 */
	public function get_excluded_products() {
		 ...
		 * @param int   $ID                Discount ID.
		 */
		return (array) apply_filters( 'cl_get_discount_excluded_products', $this->excluded_products, $this->ID );
	}
	...
	public function get_start() {
		/**
		  ...
		  * @param int    $ID    Discount ID.
		  */
		return apply_filters( 'cl_get_discount_start', $this->start, $this->ID );
	}
	...
	public function get_expiration() {
		/**
		   ...
		   * @param int   $ID          Discount ID.
		   */
		return apply_filters( 'cl_get_discount_expiration', $this->expiration, $this->ID );
	}
	...
	public function get_uses() {
		 ...
		 * @param int $ID       Discount ID.
		 */
		return (int) apply_filters( 'cl_get_discount_uses', $this->uses, $this->ID );
	}
	...
	public function get_max_uses() {
		 ...
		 * @param int $ID       Discount ID.
		 */
		return (int) apply_filters( 'cl_get_discount_max_uses', $this->max_uses, $this->ID );
	}
	...
	public function get_min_price() {
		/**
		  ...
		  * @param int   $ID        Discount ID.
		  */
		return (float) apply_filters( 'cl_get_discount_min_price', $this->min_price, $this->ID );
	}
	...
	public function get_is_single_use() {
		/**
		  ...
		  * @param int  $ID            Discount ID.
		  */
		return (bool) apply_filters( 'cl_is_discount_single_use', $this->is_single_use, $this->ID );
	}
	...
	public function get_is_not_global() {
		/**
		  ...
		  * @param int  $ID            Discount ID.
		  */
		return (bool) apply_filters( 'cl_discount_is_not_global', $this->is_not_global, $this->ID );
	}
	...
	public function get_product_condition() {
		/**
		  ...
		  * @param int    $ID                Discount ID.
		  */
		return apply_filters( 'cl_discount_product_condition', $this->product_condition, $this->ID );
	}
	...
	private function insert_discount() {
		...
		// Create a blank cl_discount post
	...
	public function save() {
		...
		if ( true == $saved ) {
			...
			do_action( 'cl_discount_save', $this->ID, $this );
		}
	...
	public function add( $args ) {
		...
		if ( ! empty( $this->ID ) && $this->exists() ) {
			 ...
			 * @param int   $ID   Discount ID.
			 */
			$meta = apply_filters( 'cl_insert_discount', $meta );
			 ...
			 * @param array $meta Discount meta.
			 */
			do_action( 'cl_pre_insert_discount', $meta );
			 ...
			 */
			do_action( 'cl_post_insert_discount', $meta, $this->ID );
	...
	public function update( $args ) {
		 ...
		 * @param int   $ID   Discount ID.
		 */
		$meta = apply_filters( 'cl_update_discount', $meta, $this->ID );
		 ...
		 * @param int   $ID   Discount ID.
		 */
		do_action( 'cl_pre_update_discount', $meta, $this->ID );
		 ...
		 * @param int   $ID   Discount ID.
		 */
		do_action( 'cl_post_update_discount', $meta, $this->ID );
	...
	public function update_status( $new_status = 'active' ) {
		 ...
		 */
		do_action( 'cl_pre_update_discount_status', $this->ID, $new_status, $this->post_status );
		 ...
		 * @param string $post_status Post status.
		 */
		do_action( 'cl_post_update_discount_status', $this->ID, $new_status, $this->post_status );
	...
	public function is_started( $set_error = true ) {
		...
		if ( $this->start ) {
			...
			if ( $start_date < time() ) {
				...
				WPERECCP()->front->error->cl_set_error( 'cl-discount-error', _x( 'This discount is invalid.', 'error shown when attempting to use a discount before its start date', 'essential-wp-real-estate' ) );
			}
		} else {
		 ...
		 * @param int  $ID     Discount ID.
		 */
		return apply_filters( 'cl_is_discount_started', $return, $this->ID );
	}
	...
	public function is_expired( $update = true ) {
		 ...
		 * @param int  $ID     Discount ID.
		 */
		return apply_filters( 'cl_is_discount_expired', $return, $this->ID );
	}
	...
	public function is_maxed_out( $set_error = true ) {
		...
		if ( $this->uses >= $this->max_uses && ! empty( $this->max_uses ) ) {
			if ( $set_error ) {
				WPERECCP()->front->error->cl_set_error( 'cl-discount-error', __( 'This discount has reached its maximum usage.', 'essential-wp-real-estate' ) );
			}
		 ...
		 * @param int  $ID     Discount ID.
		 */
		return apply_filters( 'cl_is_discount_maxed_out', $return, $this->ID );
	}
	...
	public function is_min_price_met( $set_error = true ) {
		...
		$cart_amount = cl_get_cart_discountable_subtotal( $this->ID );

		if ( (float) $cart_amount >= (float) $this->min_price ) {
			$return = true;
		} elseif ( $set_error ) {
			WPERECCP()->front->error->cl_set_error( 'cl-discount-error', sprintf( __( 'Minimum order of %s not met.', 'essential-wp-real-estate' ), cl_currency_filter( cl_format_amount( $this->min_price ) ) ) );
		}
		 ...
		 * @param int  $ID     Discount ID.
		 */
		return apply_filters( 'cl_is_discount_min_met', $return, $this->ID );
	}
	...
	public function is_product_requirements_met( $set_error = true ) {
		...
		if ( ! $return && ! empty( $product_reqs ) ) {
			switch ( $this->product_condition ) {
				case 'all':
					...
					foreach ( $product_reqs as $listing_id ) {

						if ( empty( $listing_id ) ) {
							continue;
						}

						if ( ! cl_item_in_cart( $listing_id ) ) {

							if ( $set_error ) {
								WPERECCP()->front->error->cl_set_error( 'cl-discount-error', __( 'The product requirements for this discount are not met.', 'essential-wp-real-estate' ) );
							}
		...
		if ( ! empty( $excluded_ps ) ) {
			if ( count( array_intersect( $cart_ids, $excluded_ps ) ) == count( $cart_ids ) ) {
				...
				if ( $set_error ) {
					WPERECCP()->front->error->cl_set_error( 'cl-discount-error', __( 'This discount is not valid for the cart contents.', 'essential-wp-real-estate' ) );
				}
			}
		}
		 ...
		 * @param string $product_condition Product condition.
		 */
		return (bool) apply_filters( 'cl_is_discount_products_req_met', $return, $this->ID, $this->product_condition );
	}
	...
	public function is_used( $user = '', $set_error = true ) {
		...
		if ( $this->is_single_use ) {
			...
			if ( WPERECCP()->common->dbcustomer->installed() ) {
			...
			if ( $payments ) {
				foreach ( $payments as $payment ) {
					...
					if ( is_array( $discounts ) ) {
						...
						if ( false !== $key ) {
							if ( $set_error ) {
								WPERECCP()->front->error->cl_set_error( 'cl-discount-error', __( 'This discount has already been redeemed.', 'essential-wp-real-estate' ) );
							}
		 ...
		 * @param string $user   User info.
		 */
		return apply_filters( 'cl_is_discount_used', $return, $this->ID, $user );
	}
	...
	public function is_valid( $user = '', $set_error = true ) {
		...
		if ( cl_get_cart_contents() && $this->ID ) {
			if (
			...
			WPERECCP()->front->error->cl_set_error( 'cl-discount-error', _x( 'This discount is invalid.', 'error for when a discount is invalid based on its configuration', 'essential-wp-real-estate' ) );
		}
		 ...
		 * @param string $user   User info.
		 */
		return apply_filters( 'cl_is_discount_valid', $return, $this->ID, $this->code, $user );
	}
	...
	public function is_active( $update = true, $set_error = true ) {
		...
		if ( $this->exists() ) {
			if ( $this->is_expired( $update ) ) {
				if ( defined( 'DOING_AJAX' ) && $set_error ) {
					WPERECCP()->front->error->cl_set_error( 'cl-discount-error', __( 'This discount is expired.', 'essential-wp-real-estate' ) );
				}
			} elseif ( $this->post_status == 'active' ) {
				$return = true;
			} elseif ( defined( 'DOING_AJAX' ) && $set_error ) {
				WPERECCP()->front->error->cl_set_error( 'cl-discount-error', __( 'This discount is not active.', 'essential-wp-real-estate' ) );
			}
		}
		 ...
		 * @param int  $ID     Discount ID.
		 */
		return apply_filters( 'cl_is_discount_active', $return, $this->ID );
	}
	...
	public function get_discounted_amount( $base_price ) {
		 ...
		 */
		return apply_filters( 'cl_discounted_amount', $amount );
	}
	...
	public function increase_usage() {
		 ...
		 * @param string $code Discount code.
		 */
		do_action( 'cl_discount_increase_use_count', $this->uses, $this->ID, $this->code );
	...
	public function decrease_usage() {
		 ...
		 * @param string $code Discount code.
		 */
		do_action( 'cl_discount_decrease_use_count', $this->uses, $this->ID, $this->code );
	...
	public function edit_url() {
		return esc_url(
			add_query_arg(
				...
				admin_url( 'edit.php?post_type=cl_cpt&page=cl_discounts' )
			)
		);
	}
}
