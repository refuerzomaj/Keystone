<?php
namespace  Essential\Restate\Front\Purchase\Gateways\Stripe\Classes;

use Essential\Restate\Traitval\Traitval;

/**
 * Class CL_Stripe_Rate_Limiting
 */
class CL_Stripe_Rate_Limiting {
	...
	private function actions() {
		...
		add_action( 'cl_before_purchase_form', array( $this, 'listen_for_recurring_card_errors' ), 0 );
	}
	...
	private function filters() {
		...
		add_filter( 'cl_checkout_button_purchase', array( $this, 'maybe_hide_purchase_button' ) );
	}
	...
	public function setup_log_file() {
		...
		$this->filename = wp_hash( home_url( '/' ) ) . '-cl-stripe-rate-limiting.log';
	...
	public function has_hit_card_error_limit() {
		 ...
		 */
		$max_error_count = apply_filters( 'cls_max_card_error_count', $max_error_count );
	...
	protected function update_rate_limiting_count( $blocking_id = '', $current_count = 0 ) {
		 ...
		 */
		$expiration_in_seconds = apply_filters( 'cls_card_error_timeout', $expiration_in_seconds );
	...
	public function card_error_checks_enabled() {
		 ...
		 */
		$checks_enabled = apply_filters( 'cls_card_error_checking_enabled', true );
	 ...
	 *
	 * ID is the IP address of the visitor. Prepends the value with `cls_card_errors_` for use with the transient system.
	...
	public function listen_for_recurring_card_errors() {
		...
		if ( isset( $errors['cl_recurring_stripe_error'] ) && ! empty( $errors['cl_recurring_stripe_error'] ) ) {
