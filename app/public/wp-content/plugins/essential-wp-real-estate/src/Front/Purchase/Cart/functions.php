...
function cl_get_cart_contents() {
	return WPERECCP()->front->cart->get_contents();
}
...
function cl_get_cart_quantity() {
	return WPERECCP()->front->cart->get_quantity();
}
...
function cl_remove_from_cart( $cart_key ) {
	return WPERECCP()->front->cart->remove( $cart_key );
}
...
function cl_item_in_cart( $listing_id = 0, $options = array() ) {
	return WPERECCP()->front->cart->is_item_in_cart( $listing_id, $options );
}
...
function cl_get_item_position_in_cart( $listing_id = 0, $options = array() ) {
	return WPERECCP()->front->cart->get_item_position( $listing_id, $options );
}
...
function cl_set_cart_item_quantity( $listing_id = 0, $quantity = 1, $options = array() ) {
	return WPERECCP()->front->cart->set_item_quantity( $listing_id, $quantity, $options );
}
...
function cl_get_cart_item_quantity( $listing_id = 0, $options = array() ) {
	return WPERECCP()->front->cart->get_item_quantity( $listing_id, $options );
}
...
function cl_cart_item_price( $item_id = 0, $options = array() ) {
	return WPERECCP()->front->cart->item_price( $item_id, $options );
}
...
function cl_get_cart_item_price( $listing_id = 0, $options = array(), $remove_tax_from_inclusive = false ) {
	return WPERECCP()->front->cart->get_item_price( $listing_id, $options, $remove_tax_from_inclusive );
}
...
function cl_get_cart_item_final_price( $item_key = 0 ) {
	return WPERECCP()->front->cart->get_item_final_price( $item_key );
}
...
function cl_get_cart_item_tax( $listing_id = 0, $options = array(), $subtotal = '' ) {
	return WPERECCP()->front->cart->get_item_tax( $listing_id, $options, $subtotal );
}
...
function cl_get_cart_item_price_id( $item = array() ) {
	return WPERECCP()->front->cart->get_item_price_id( $item );
}
...
function cl_get_cart_item_price_name( $item = array() ) {
	return WPERECCP()->front->cart->get_item_price_name( $item );
}
...
function cl_cart_subtotal() {
	return WPERECCP()->front->cart->subtotal();
}
...
function cl_get_cart_discountable_subtotal( $code_id ) {
	return WPERECCP()->front->cart->get_discountable_subtotal( $code_id );
}
...
function cl_get_cart_items_subtotal( $items ) {
	return WPERECCP()->front->cart->get_items_subtotal( $items );
}
/**
...
function cl_cart_total( $echo = true ) {
	if ( ! $echo ) {
		return WPERECCP()->front->cart->total( $echo );
	}

	WPERECCP()->front->cart->total( $echo );
}
...
function cl_get_cart_fee_tax() {
	return WPERECCP()->front->cart->get_tax_on_fees();
}
...
function cl_cart_tax( $echo = false ) {
	if ( ! $echo ) {
		return WPERECCP()->front->cart->tax( $echo );
	} else {
		WPERECCP()->front->cart->tax( $echo );
	}
}
...
function cl_remove_item_url( $cart_key ) {
	return WPERECCP()->front->cart->remove_item_url( $cart_key );
}
...
function cl_remove_cart_fee_url( $fee_id = '' ) {
	return WPERECCP()->front->cart->remove_fee_url( $fee_id );
}
 ...
 * @param $purchase_data
 *
 * @uses WPERECCP()->front->session->set()
 */
 ...
 *
 * @since 1.1.5
 * @uses WPERECCP()->front->session->get()
 * @return mixed array | false
 */
function cl_get_purchase_session() {
	return WPERECCP()->front->session->get( 'cl_purchase' );
}
...
function cl_is_cart_saving_disabled() {
	 return ! WPERECCP()->front->cart->is_saving_enabled();
}
...
function cl_is_cart_saved() {
	return WPERECCP()->front->cart->is_saved();
}
...
function cl_save_cart() {
	return WPERECCP()->front->cart->save();
}
...
function cl_restore_cart() {
	return WPERECCP()->front->cart->restore();
}
...
function cl_get_cart_token() {
	return WPERECCP()->front->cart->get_token();
}
...
function cl_generate_cart_token() {
	 return WPERECCP()->front->cart->generate_token();
}
