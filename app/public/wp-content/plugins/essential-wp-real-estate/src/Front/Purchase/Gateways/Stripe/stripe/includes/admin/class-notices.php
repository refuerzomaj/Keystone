...
class CL_Stripe_Admin_Notices {
	...
	/**
	 * CL_Stripe_Admin_Notices
	 *
	 * @param CL_Stripe_Notices_Registry $registry Notices registry.
	 */
	...
	public function get_dismissed_option_name( $notice_id ) {
		...
		switch ( $notice_id ) {
			case 'stripe-connect':
				$option_name = 'cls_stripe_connect_intro_notice_dismissed';
				break;
			default:
				$option_name = sprintf( 'cls_notice_%1$s_dismissed', $notice_id );
		}
	...
	public function build( $notice_id ) {
		...
		$output = sprintf(
			'<div id="cls-%1$s-notice" class="%2$s" data-id="%1$s" data-nonce="%3$s" role="alert">%4$s</div>',
			esc_attr( $notice_id ),
			esc_attr( implode( ' ', $classes ) ),
			wp_create_nonce( "cls-dismiss-{$notice_id}-nonce" ),
			$message
		);
