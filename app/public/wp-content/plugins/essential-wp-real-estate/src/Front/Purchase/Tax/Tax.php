<?php
namespace  Essential\Restate\Front\Purchase\Tax;
...
class Tax {
	...
	function cl_get_tax_rate( $country = false, $state = false ) {
		...
		if ( empty( $country ) ) {
			...
			$country = ! empty( $country ) ? $country : WPERECCP()->front->country->cl_get_shop_country();
		}
		...
		if ( empty( $state ) ) {
			...
			$state = ! empty( $state ) ? $state : WPERECCP()->front->country->cl_get_shop_state();
		}
	...
	function cl_sales_tax_for_year( $year = null ) {
		echo WPERECCP()->common->formatting->cl_currency_filter( WPERECCP()->common->formatting->cl_format_amount( $this->cl_get_sales_tax_for_year( $year ) ) );
	}
