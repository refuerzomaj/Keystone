...
export function paymentMethods() {
	...
	const existingPaymentMethods = document.querySelectorAll( '.cl-stripe-existing-card' );
	...
	if ( 0 !== existingPaymentMethods.length ) {
		...
		let currentPaymentMethod = document.querySelector( '.cl-stripe-existing-card:checked' );

		if ( ! currentPaymentMethod ) {
			currentPaymentMethod = document.querySelector( '.cl-stripe-existing-card:first-of-type' );
...
function getBillingFieldsToggle() {
	return document.getElementById( 'cl-stripe-update-billing-address' );
}
...
function toggleBillingFields( isVisible ) {
	const updateAddressWrapperEl = document.querySelector( '.cl-stripe-update-billing-address-wrapper' );
	...
	const billingAddressPreview = document.querySelector( '.cl-stripe-update-billing-address-current' );
...
function onPaymentSourceChange( paymentSource ) {
	...
	const newCardForm = document.querySelector( '.cl-stripe-new-card' );
	const billingAddressToggle = document.querySelector( '.cl-stripe-update-billing-address-wrapper' );
	...
	// @todo don't be lazy.
	$( '.cl-stripe-card-radio-item' ).removeClass( 'selected' );
	$( paymentSource ).closest( '.cl-stripe-card-radio-item' ).addClass( 'selected' );
	...
	if ( isNew ) {
		...
		// Recalculate taxes.
		if ( window.CL_Checkout.recalculate_taxes ) {
			window.CL_Checkout.recalculate_taxes();
		}
		...
		$( document ).ajaxSuccess( function( event, xhr, options ) {
			...
			if (
				options.data.includes( 'action=cl_get_shop_states' ) &&
				...
				if ( stateField ) {
					...
					// Recalculate taxes.
					if ( window.CL_Checkout.recalculate_taxes ) {
						window.CL_Checkout.recalculate_taxes( stateField.value );
					}
				}
			}
		} );
		...
		if ( billingAddressPreview ) {
			...
			document.querySelector( '.cl-stripe-update-billing-address-brand' ).innerHTML = brand;
			document.querySelector( '.cl-stripe-update-billing-address-last4' ).innerHTML = last4;
		}
	}
}
