...
function hideAndSwitchGateways() {
	...
	const gatewayRadioEl = document.getElementById( 'cl-gateway-option-stripe-prb' );
	...
	const gateways = document.querySelectorAll( '.cl-gateway-option' );
	const nextGateway = gateways[0];
	const nextGatewayInput = nextGateway.querySelector( 'input' );

	// Toggle radio.
	nextGatewayInput.checked = true;
	nextGateway.classList.add( 'cl-gateway-option-selected' );

	// Load gateway.
	cl_load_gateway( nextGatewayInput.value );

	// Hide wrapper.
	if ( 1 === gateways.length ) {
		document.getElementById( 'cl_payment_mode_select_wrap' ).remove();
	}
}
...
function onClick( event ) {
	const errorContainer = document.getElementById( 'cls-prb-error-wrap' );
	const {
		checkout_agree_to_terms,
		checkout_agree_to_privacy,
	} = cl_stripe_vars;

	const termsEl = document.getElementById( 'cl_agree_to_terms' );
	...
	const privacyEl = document.getElementById( 'cl-agree-to-privacy-policy' );
...
async function onChange( paymentRequest, checkoutForm ) {
	try {
		...
		outputNotice( {
			errorMessage: '',
			errorContainer: document.getElementById( 'cls-prb-checkout' ), 
...
function onPaymentMethodError( event, error, checkoutForm ) {
	...
	// Remove spinner.
	const spinner = checkoutForm.querySelector( '.cls-prb-spinner' );
...
async function onPaymentMethod( paymentRequest, checkoutForm, event ) {
	try {
		...
		[ 'cl-loading-ajax', 'cl-loading', 'cls-prb-spinner' ].forEach(
 ...
 * 
 * @param {Event} e cl_discount_applied event.
...
function bindEvents( paymentRequest, checkoutForm ) {
	...
	$body.on( 'cl_quantity_updated', () => onChange( paymentRequest, checkoutForm ) );

	// Discounts have changed.
	$body.on( 'cl_discount_applied', () => onChange( paymentRequest, checkoutForm ) );
	$body.on( 'cl_discount_removed', () => onChange( paymentRequest, checkoutForm ) );
	...
	$body.on( 'cl_discount_applied', onApplyDiscount );
}
...
function mount( element ) {
	...
	const checkoutForm = document.getElementById( 'cl_checkout_form_wrap' );
	...
	try {
		...
		paymentRequest.canMakePayment()
			...
			.then( function( result ) {
				...
				checkoutForm.classList.add( 'cl-prb--is-active' );
		...
		outputNotice( {
			errorMessage: error.message,
			errorContainer: document.querySelector( '#cls-prb-checkout' ), 
			errorContainerReplace: false,
		} );
	}
};
...
export function setup() {
	 ...
	 */
	jQuery( document.body ).on( 'cl_gateway_loaded', ( e, gateway ) => {
		...
		const prbEl = document.querySelector( '.cls-prb.cls-prb--checkout' );
