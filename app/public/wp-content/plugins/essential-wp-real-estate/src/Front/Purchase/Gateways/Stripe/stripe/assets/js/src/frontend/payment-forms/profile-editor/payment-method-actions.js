...
export function paymentMethodActions() {
	// Update.
	forEach( document.querySelectorAll( '.cl-stripe-update-card' ), function( updateButton ) {
		updateButton.addEventListener( 'click', onToggleUpdateForm );
	} );

	forEach( document.querySelectorAll( '.cl-stripe-cancel-update' ), function( cancelButton ) {
	...
	// Delete.
	forEach( document.querySelectorAll( '.cl-stripe-delete-card' ), function( deleteButton ) {
		deleteButton.addEventListener( 'click', onDeletePaymentMethod );
	} );

	// Set Default.
	forEach( document.querySelectorAll( '.cl-stripe-default-card' ), function( setDefaultButton ) {
...
function onUpdatePaymentMethod( e ) {
	...
	paymentMethodAction( 'cls_update_payment_method', e.target.dataset.source, data )
		.fail( function( response ) {
...
function onDeletePaymentMethod( e ) {
	e.preventDefault();
	const loading = '<span class="cl-loading-ajax cl-loading"></span>';
	const linkText = e.target.innerText;
	e.target.innerHTML = loading;

	paymentMethodAction( 'cls_delete_payment_method', e.target.dataset.source )
		.fail( function( response ) {
...
function onSetDefaultPaymentMethod( e ) {
	...
	paymentMethodAction( 'cls_set_payment_method_default', e.target.dataset.source )
		.fail( function( response ) {
			e.target.innerText = linkText;
		} );
}
