...
export function generateNotice( message, type = 'error' ) {
	...
	notice.classList.add( 'cl-alert' );
	notice.classList.add( 'cl-stripe-alert' );
	notice.style.clear = 'both';

	if ( 'error' === type ) {
		notice.classList.add( 'cl-alert-error' );
	} else {
		notice.classList.add( 'cl-alert-success' );
	}

	notice.innerHTML = message || cl_stripe_vars.generic_error;
