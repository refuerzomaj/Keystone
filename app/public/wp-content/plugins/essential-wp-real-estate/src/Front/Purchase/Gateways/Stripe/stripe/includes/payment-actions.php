...
function cls_process_purchase_form( $purchase_data ) {
	...
	try {
		...
		if ( ! $payment_method_id ) {
			throw new \CL_Stripe_Gateway_Exception(
				esc_html__(
					'Unable to locate payment method 1. Please try again with a new payment method.',
					'essential-wp-real-estate'
				)
			);
		}
...
function cls_complete_payment_authorization() {
	...
	try {
		...
		$nonce = isset( $_POST['cls-complete-payment-authorization'] )
			? cl_sanitization( $_POST['cls-complete-payment-authorization'] )
			: '';
