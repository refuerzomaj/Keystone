...
function cl_stripe_process_card_update() {
	...
	if ( ! $enabled ) {
		wp_send_json_error(
			array(
				'message' => __( 'This feature is not available at this time.', 'essential-wp-real-estate' ),
			)
		);
	}
	...
	if ( empty( $payment_method ) ) {
		wp_send_json_error(
			array(
				'message' => __( 'Error updating card.', 'essential-wp-real-estate' ),
			)
		);
	}
	...
	try {
		...
		wp_send_json_success(
			array(
				'message' => esc_html__( 'Card successfully updated.', 'essential-wp-real-estate' ),
			)
		);
	} catch ( \Exception $e ) {
		wp_send_json_error(
...
function cl_stripe_process_card_default() {
	...
	try {
		...
		wp_send_json_success(
			array(
				'message' => esc_html__( 'Card successfully set as default.', 'essential-wp-real-estate' ),
			)
		);
	} catch ( \Exception $e ) {
		wp_send_json_error(
...
function cl_stripe_process_card_delete() {
	...
	if ( ! $should_remove['remove'] ) {
		wp_send_json_error(
			array(
				'message' => esc_html__( 'This feature is not available at this time.', 'essential-wp-real-estate' ),
			)
		);
	}
	...
	try {
		...
		wp_send_json_success(
			array(
				'message' => esc_html__( 'Card successfully removed.', 'essential-wp-real-estate' ),
			)
		);
	} catch ( \Exception $e ) {
		wp_send_json_error(
...
function cls_add_payment_method() {
	...
	if ( cl_stripe()->rate_limiting->has_hit_card_error_limit() ) {
		...
		wp_send_json_error(
			array(
				'message' => __( 'Unable to update your account at this time, please try again later', 'essential-wp-real-estate' ),
			)
		);
	}
	...
	if ( ! $payment_method_id ) {
		wp_send_json_error(
			array(
				'message' => __( 'Missing card ID.', 'essential-wp-real-estate' ),
			)
		);
	}
	...
	if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'cl-stripe-add-card' ) ) {
		wp_send_json_error(
			array(
				'message' => __( 'Error adding card.', 'essential-wp-real-estate' ),
			)
		);
	}
	...
	try {
		...
		wp_send_json_success(
			array(
				'message' => esc_html__( 'Card successfully added.', 'essential-wp-real-estate' ),
			)
		);
	} catch ( \Exception $e ) {
		// Increase the card error count.
