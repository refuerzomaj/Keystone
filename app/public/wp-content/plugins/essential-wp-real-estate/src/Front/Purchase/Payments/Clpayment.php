...
class Clpayment {
	...
	/**
	 * Array of listing for this payment
	 *
	 * @since  2.5
	 * @var array
	 */
	...
	/**
	 * Allows the files for this payment to be listinged unlimited times (when listing limits are enabled)
	 *
	 * @since  2.5
	 * @var boolean
	 */
	protected $has_unlimited_listing = false;
	...
	public function __set( $key, $value ) {
		$ignore = array( 'listing', 'cart_details', 'fees', '_ID' );
	...
	private function setup_payment( $payment_id ) {
		...
		if ( 'cl_payment' !== $payment->post_type ) {
			return false;
		}

		// Allow extensions to perform actions before the payment is loaded
		do_action( 'cl_pre_setup_payment', $this, $payment_id );
		...
		$all_payment_statuses  = cl_get_payment_statuses();
		$this->status_nicename = array_key_exists( $this->status, $all_payment_statuses ) ? $all_payment_statuses[ $this->status ] : ucfirst( $this->status );

		// Items
		$this->fees         = $this->setup_fees();
		$this->cart_details = $this->setup_cart_details();
		$this->listing      = $this->setup_listing();
		...
		$this->has_unlimited_listing = $this->setup_has_unlimited();

		// Allow extensions to add items to this object via hook
		do_action( 'cl_setup_payment', $this, $payment_id );
	...
	private function insert_payment() {
		...
		if ( empty( $this->key ) ) {
			...
			$this->key            = strtolower( md5( $this->email . date( 'Y-m-d H:i:s' ) . $auth_key . uniqid( 'cl', true ) ) );  // Unique key
		...
		if ( ! empty( $payment_id ) ) {
			...
			/**
			 * This run of the cl_payment_meta filter is for backwards compatibility purposes. The filter will also run in the Clpayment::save
			 ...
			 * by cl_insert_payment().
			 */
			$this->payment_meta = apply_filters( 'cl_payment_meta', $this->payment_meta, $payment_data );
			...
			if ( cl_admin_get_option( 'enable_sequential' ) ) {
				$number       = cl_get_next_payment_number();
				$this->number = cl_format_payment_number( $number );
				$this->update_meta( '_cl_payment_number', $this->number );
				update_option( 'cl_last_payment_number', $number );
			}

			$this->update_meta( '_cl_payment_meta', $this->payment_meta );
			$this->new = true;
		}
	...
	public function save() {
		...
		if ( ! empty( $this->pending ) ) {
			...
			foreach ( $this->pending as $key => $value ) {
				switch ( $key ) {
					case 'listing':
						// Update totals for pending listing
						foreach ( $this->pending[ $key ] as $item ) {
							...
							switch ( $item['action'] ) {
								...
								case 'add':
									...
									if ( 'publish' === $this->status || 'complete' === $this->status || 'revoked' === $this->status ) {
										...
										while ( $y < $item['quantity'] ) {
											WPERECCP()->front->listingsaction->cl_record_sale_in_log( $item['id'], $this->ID, $price_id, $log_date );
											$y++;
										}
										...
										$listing->increase_sales( $item['quantity'] );
										$listing->increase_earnings( $increase_earnings );
								...
								case 'remove':
									...
									$found_logs = $cl_logs->get_connected_logs( $log_args );
									...
									if ( 'publish' === $this->status || 'complete' === $this->status || 'revoked' === $this->status ) {
										$listing = new Listings( $item['id'] );
										$listing->decrease_sales( $item['quantity'] );
										...
										$listing->decrease_earnings( $decrease_amount );
								...
								case 'modify':
									if ( 'publish' === $this->status || 'complete' === $this->status || 'revoked' === $this->status ) {
										...
										if ( $item['previous_data']['quantity'] != $item['quantity'] ) {
											...
											$existing_logs = $cl_logs->get_connected_logs( $log_args );
					...
					case 'gateway':
						$this->update_meta( '_cl_payment_gateway', $this->gateway );
						break;

					case 'mode':
						$this->update_meta( '_cl_payment_mode', $this->mode );
						break;

					case 'transaction_id':
						$this->update_meta( '_cl_payment_transaction_id', $this->transaction_id );
						break;

					case 'ip':
						$this->update_meta( '_cl_payment_user_ip', $this->ip );
						break;

					case 'customer_id':
						$this->update_meta( '_cl_payment_customer_id', $this->customer_id );
						$customer = new Customer( $this->customer_id );
						$customer->attach_payment( $this->ID, false );
						break;

					case 'user_id':
						$this->update_meta( '_cl_payment_user_id', $this->user_id );
					...
					case 'email':
						...
						$this->update_meta( '_cl_payment_user_email', $this->email );
						break;

					case 'key':
						$this->update_meta( '_cl_payment_purchase_key', $this->key );
						break;

					case 'tax_rate':
						$this->update_meta( '_cl_payment_tax_rate', $this->tax_rate );
						break;
					...
					case 'completed_date':
						$this->update_meta( '_cl_completed_date', $this->completed_date );
						break;

					case 'has_unlimited_listing':
						$this->update_meta( '_cl_payment_unlimited_listing', $this->has_unlimited_listing );
						break;
					...
					default:
						 ...
						 */
						do_action( 'cl_payment_save', $this, $key );
						break;
				}
			}
			...
			if ( ! $this->in_process() ) {
				...
				if ( $total_change < 0 ) {
					...
					cl_decrease_total_earnings( $total_change );
				} elseif ( $total_change > 0 ) {

					// Increase the customer's purchase stats
					$customer->increase_value( $total_change );
					cl_increase_total_earnings( $total_change );
				}
			}

			$this->update_meta( '_cl_payment_total', $this->total );
			$this->update_meta( '_cl_payment_tax', $this->tax );

			$this->listing = array_values( $this->listing );
			...
			// Do some merging of user_info before we merge it all, to honor the cl_payment_meta filter
			...
			$merged_meta  = apply_filters( 'cl_payment_meta', $merged_meta, $payment_data );

			// Only save the payment meta if it's changed
			if ( md5( serialize( $meta ) ) !== md5( serialize( $merged_meta ) ) ) {
				$updated = $this->update_meta( '_cl_payment_meta', $merged_meta );
				if ( false !== $updated ) {
					$saved = true;
				}
			}
		...
		if ( true === $saved ) {
			 ...
			 */
			do_action( 'cl_payment_saved', $this->ID, $this );
		}

		/**
		 * Update the payment in the object cache
		 */
		$cache_key = md5( 'cl_payment' . $this->ID );
	...
	/**
	 * Add a listing to a given payment
	 *
	 * @since 2.5
	 * @param int   $listing_id The listing to add
	 * @param array $args Other arguments to pass to the function
	 * @param array $options List of listing options
	 * @return bool True when successful, false otherwise
	 */
	public function add_listing( $listing_id = 0, $args = array(), $options = array() ) {
		$listing = new Listings( $listing_id );

		// Bail if this post isn't a listing
		// if (!$listing || $listing->post_type !== 'cl_cpt') {
		// return false;
		// }
		...
		$args = wp_parse_args( apply_filters( 'cl_payment_add_listing_args', $args, $listing->ID ), $defaults );
		...
		if ( false !== $args['item_price'] ) {
			...
			if ( cl_has_variable_prices( $listing->ID ) ) {
				$prices = get_post_meta( $listing->ID, 'cl_variable_prices', true );

				if ( $args['price_id'] && array_key_exists( $args['price_id'], (array) $prices ) ) {
					$item_price = $prices[ $args['price_id'] ]['amount'];
				} else {
					$item_price       = cl_get_lowest_price_option( $listing->ID );
					$args['price_id'] = cl_get_lowest_price_id( $listing->ID );
				}
			} else {
				$item_price = cl_get_listing_price( $listing->ID );
			}
		}

		// Sanitizing the price here so we don't have a dozen calls later
		$item_price = WPERECCP()->common->formatting->cl_sanitize_amount( $item_price );
		$quantity   = cl_item_quantities_enabled() ? absint( $args['quantity'] ) : 1;
		$amount     = round( $item_price * $quantity, WPERECCP()->common->formatting->cl_currency_decimal_filter() );

		// Setup the listing meta item
		$new_listing = array(
			'id'       => $listing->ID,
			'quantity' => $quantity,
		);
		...
		$new_listing['options'] = $options;

		$this->listing[] = $new_listing;
		...
		if ( ! empty( $args['fees'] ) && is_array( $args['fees'] ) ) {
			...
			$fees = round( $fees, WPERECCP()->common->formatting->cl_currency_decimal_filter() );
		}
		...
		$added_listing           = end( $this->cart_details );
		$added_listing['action'] = 'add';

		$this->pending['listing'][] = $added_listing;
	...
	/**
	 * Remove a listing from the payment
	 *
	 * @since  2.5
	 * @param  int   $listing_id The listing ID to remove
	 * @param  array $args        Arguments to pass to identify (quantity, amount, price_id)
	 * @return bool               If the item was removed or not
	 */
	public function remove_listing( $listing_id, $args = array() ) {
		...
		/**
		 * Bail if this post isn't a listing post type.
		 *
		 * We need to allow this to process though for a missing post ID, in case it's a listing that was deleted.
		 */
		if ( ! empty( $listing->ID ) && $listing->post_type !== 'listing' ) {
			return false;
		}

		foreach ( $this->listing as $key => $item ) {

			if ( (int) $listing_id !== (int) $item['id'] ) {
				continue;
			}
			...
			if ( false !== $args['price_id'] ) {
				...
				if ( ! empty( $cart_item ) ) {
					...
					// If the cart index item isn't the same listing ID, don't remove it
			...
			$item_quantity = $this->listing[ $key ]['quantity'];

			if ( $item_quantity > $args['quantity'] ) {

				$this->listing[ $key ]['quantity'] -= $args['quantity'];
				break;
			} else {

				unset( $this->listing[ $key ] );
				break;
			}
		}
		...
		if ( false === $args['cart_index'] ) {
			...
			if ( (int) $this->cart_details[ $cart_index ]['id'] !== (int) $listing_id ) {
				return false; // We still need the proper listing ID to be sure.
			}
		...
		if ( $orig_quantity > $args['quantity'] ) {
			...
			$total_reduced = round( $item_price * $args['quantity'], WPERECCP()->common->formatting->cl_currency_decimal_filter() );
			$tax_reduced   = round( ( $tax / $orig_quantity ) * $args['quantity'], WPERECCP()->common->formatting->cl_currency_decimal_filter() );
		...
		$this->pending['listing'][] = $pending_args;
	...
	public function modify_cart_item( $cart_index = false, $args = array() ) {
		...
		$allowed_items = apply_filters(
			'cl_allowed_cart_item_modifications',
			array(
				'item_price',
				'tax',
				'discount',
		...
		$merged_item['item_price'] = WPERECCP()->common->formatting->cl_sanitize_amount( $merged_item['item_price'] );
		$new_subtotal              = floatval( $merged_item['item_price'] ) * $merged_item['quantity'];
		$merged_item['tax']        = WPERECCP()->common->formatting->cl_sanitize_amount( $merged_item['tax'] );
		$merged_item['price']      = WPERECCP()->front->tax->cl_prices_include_tax() ? $new_subtotal : $new_subtotal + $merged_item['tax'];
		...
		$modified_listing                  = $merged_item;
		$modified_listing['action']        = 'modify';
		$modified_listing['previous_data'] = $current_args;

		$this->pending['listing'][] = $modified_listing;

		if ( $new_subtotal > $current_args['subtotal'] ) {
			$this->increase_subtotal( ( $new_subtotal - $modified_listing['discount'] ) - $current_args['subtotal'] );
		} else {
			$this->decrease_subtotal( $current_args['subtotal'] - ( $new_subtotal - $modified_listing['discount'] ) );
		}

		if ( $modified_listing['tax'] > $current_args['tax'] ) {
			$this->increase_tax( $modified_listing['tax'] - $current_args['tax'] );
		} else {
			$this->decrease_tax( $current_args['tax'] - $modified_listing['tax'] );
		}
	...
	public function get_fees( $type = 'all' ) {
		...
		return apply_filters( 'cl_get_payment_fees', $fees, $this->ID, $this );
	}
	...
	public function add_note( $note = false ) {
		...
		cl_insert_payment_note( $this->ID, esc_html( $note ) );
	}
	...
	public function update_status( $status = false ) {
		...
		$do_change = apply_filters( 'cl_should_update_payment_status', true, $this->ID, $status, $old_status );

		$updated = false;

		if ( $do_change ) {

			do_action( 'cl_before_payment_status_change', $this->ID, $status, $old_status );
			...
			$updated = wp_update_post( apply_filters( 'cl_update_payment_status_fields', $update_fields ) );
			...
			do_action( 'cl_update_payment_status', $this->ID, $status, $old_status );
		}
	 ...
	 */
	public function get_meta( $meta_key = '_cl_payment_meta', $single = true ) {

		$meta = get_post_meta( $this->ID, $meta_key, $single );
		if ( $meta_key === '_cl_payment_meta' ) {
			...
			if ( is_array( $meta ) && isset( $meta[0] ) ) {
				...
				update_post_meta( $this->ID, '_cl_payment_meta', $meta );
			}
		...
		$meta = apply_filters( 'cl_get_payment_meta_' . $meta_key, $meta, $this->ID );
		...
		return apply_filters( 'cl_get_payment_meta', $meta, $this->ID, $meta_key );
	}
	...
	public function update_meta( $meta_key = '', $meta_value = '', $prev_value = '' ) {
		...
		if ( '_cl_payment_purchase_key' == $meta_key ) {

			$current_meta        = $this->get_meta();
			$current_meta['key'] = $meta_value;
			update_post_meta( $this->ID, '_cl_payment_meta', $current_meta );
		} elseif ( $meta_key == 'key' || $meta_key == 'date' ) {
		...
		} elseif ( $meta_key == 'email' || $meta_key == '_cl_payment_user_email' ) {

			$meta_value = apply_filters( 'cl_cl_update_payment_meta_' . $meta_key, $meta_value, $this->ID );
			update_post_meta( $this->ID, '_cl_payment_user_email', $meta_value );

			$current_meta = $this->get_meta( '_cl_payment_meta' );
		...
		$meta_value = apply_filters( 'cl_update_payment_meta_' . $meta_key, $meta_value, $this->ID );
	...
	public function is_recoverable() {
		...
		$recoverable_statuses = apply_filters( 'cl_recoverable_payment_statuses', array( 'pending', 'abandoned', 'failed' ) );
	...
	public function get_recovery_url() {
		...
		$recovery_url = add_query_arg(
			array(
				'cl_action'  => 'recover_payment',
				'payment_id' => $this->ID,
			),
			cl_get_checkout_uri()
		);

		return apply_filters( 'cl_payment_recovery_url', $recovery_url, $this );
	}
	...
	private function process_refund() {
		...
		$process_refund = apply_filters( 'cl_should_process_refund', $process_refund, $this );

		if ( false === $process_refund ) {
			return;
		}

		do_action( 'cl_pre_refund_payment', $this );

		$decrease_store_earnings = apply_filters( 'cl_decrease_store_earnings_on_refund', true, $this );
		$decrease_customer_value = apply_filters( 'cl_decrease_customer_value_on_refund', true, $this );
		$decrease_purchase_count = apply_filters( 'cl_decrease_customer_purchase_count_on_refund', true, $this );
		...
		do_action( 'cl_post_refund_payment', $this );
	}
	...
	private function process_failure() {
		...
		foreach ( $discounts as $discount ) {
			cl_decrease_discount_usage( $discount );
		}
	}
	...
	private function process_pending() {
		...
		// Allow extensions to filter for their own payment types, Example: Recurring Payments
		$process_pending = apply_filters( 'cl_should_process_pending', $process_pending, $this );

		if ( false === $process_pending ) {
			return;
		}

		$decrease_store_earnings = apply_filters( 'cl_decrease_store_earnings_on_pending', true, $this );
		$decrease_customer_value = apply_filters( 'cl_decrease_customer_value_on_pending', true, $this );
		$decrease_purchase_count = apply_filters( 'cl_decrease_customer_purchase_count_on_pending', true, $this );

		$this->maybe_alter_stats( $decrease_store_earnings, $decrease_customer_value, $decrease_purchase_count );
		$this->delete_sales_logs();

		$this->completed_date = false;
		$this->update_meta( '_cl_completed_date', '' );
	...
	private function maybe_alter_stats( $alter_store_earnings, $alter_customer_value, $alter_customer_purchase_count ) {
		...
		cl_undo_purchase( false, $this->ID );

		// Decrease store earnings
		if ( true === $alter_store_earnings ) {
			cl_decrease_total_earnings( $this->total );
		}
	...
	private function setup_completed_date() {
		...
		$date = ( $date = $this->get_meta( '_cl_completed_date', true ) ) ? $date : $payment->date;
	...
	private function setup_mode() {
		 return $this->get_meta( '_cl_payment_mode' );
	}
	...
	private function setup_total() {
		$amount = $this->get_meta( '_cl_payment_total', true );

		if ( empty( $amount ) && '0.00' != $amount ) {
			$meta = $this->get_meta( '_cl_payment_meta', true );
	...
	private function setup_tax() {
		$tax = $this->get_meta( '_cl_payment_tax', true );
	...
	private function setup_tax_rate() {
		 return $this->get_meta( '_cl_payment_tax_rate', true );
	}
	...
	private function setup_subtotal() {
		...
		if ( is_array( $cart_details ) ) {
			...
			$subtotal  = $this->total;
			$tax       = WPERECCP()->front->tax->cl_use_taxes() ? $this->tax : 0;
			$subtotal -= $tax;
		}
	...
	private function setup_currency() {
		 $currency = isset( $this->payment_meta['currency'] ) ? $this->payment_meta['currency'] : apply_filters( 'cl_payment_currency_default', WPERECCP()->common->options->cl_get_currency(), $this );
		return $currency;
	}
	...
	private function setup_gateway() {
		$gateway = $this->get_meta( '_cl_payment_gateway', true );
		return $gateway;
	}
	...
	private function setup_transaction_id() {
		$transaction_id = $this->get_meta( '_cl_payment_transaction_id', true );

		if ( empty( $transaction_id ) || (int) $transaction_id === (int) $this->ID ) {

			$gateway        = $this->gateway;
			$transaction_id = apply_filters( 'cl_get_payment_transaction_id-' . $gateway, $this->ID );
		}
	...
	private function setup_ip() {
		$ip = $this->get_meta( '_cl_payment_user_ip', true );
		return $ip;
	}
	...
	private function setup_customer_id() {
		$customer_id = $this->get_meta( '_cl_payment_customer_id', true );
		return $customer_id;
	}
	...
	private function setup_user_id() {
		$user_id  = $this->get_meta( '_cl_payment_user_id', true );
		$customer = new Customer( $this->customer_id );
		...
		if ( ! empty( $customer->user_id ) && ( empty( $user_id ) || (int) $user_id !== (int) $customer->user_id ) ) {
			...
			$this->update_meta( '_cl_payment_user_id', $user_id );
		}
	...
	private function setup_email() {
		$email = $this->get_meta( '_cl_payment_user_email', true );

		if ( empty( $email ) ) {
			$email = WPERECCP()->common->dbcustomer->get_column( 'email', $this->customer_id );
		}
	...
	private function setup_payment_key() {
		$key = $this->get_meta( '_cl_payment_purchase_key', true );
		return $key;
	}
	...
	private function setup_payment_number() {
		...
		if ( cl_admin_get_option( 'enable_sequential' ) ) {
			...
			$number = $this->get_meta( '_cl_payment_number', true );
	...
	/**
	 * Setup the listing array
	 *
	 * @since  2.5
	 * @return array               listings associated with this payment
	 */
	private function setup_listing() {
		$listings = isset( $this->payment_meta['listing'] ) ? maybe_unserialize( $this->payment_meta['listing'] ) : array();
		return $listings;
	}

	/**
	 * Setup the Unlimited listing setting
	 *
	 * @since  2.5
	 * @return bool If this payment has unlimited listing
	 */
	private function setup_has_unlimited() {
		$unlimited = (bool) $this->get_meta( '_cl_payment_unlimited_listing', true );
		return $unlimited;
	}
	...
	private function get_cart_details() {
		return apply_filters( 'cl_payment_cart_details', $this->cart_details, $this->ID, $this );
	}
	...
	private function get_completed_date() {
		 return apply_filters( 'cl_payment_completed_date', $this->completed_date, $this->ID, $this );
	}
	...
	private function get_tax() {
		return apply_filters( 'cl_get_payment_tax', $this->tax, $this->ID, $this );
	}
	...
	private function get_subtotal() {
		return apply_filters( 'cl_get_payment_subtotal', $this->subtotal, $this->ID, $this );
	}
	...
	private function get_discounts() {
		return apply_filters( 'cl_payment_discounts', $this->discounts, $this->ID, $this );
	}
	...
	private function get_discounted_amount() {
		...
		return floatval( apply_filters( 'cl_payment_discounted_amount', $total - ( $fees + $tax ), $this ) );
	}
	...
	private function get_currency() {
		return apply_filters( 'cl_payment_currency_code', $this->currency, $this->ID, $this );
	}
	...
	private function get_gateway() {
		return apply_filters( 'cl_payment_gateway', $this->gateway, $this->ID, $this );
	}
	...
	private function get_transaction_id() {
		 return apply_filters( 'cl_get_payment_transaction_id', $this->transaction_id, $this->ID, $this );
	}
	...
	private function get_ip() {
		 return apply_filters( 'cl_payment_user_ip', $this->ip, $this->ID, $this );
	}
	...
	private function get_customer_id() {
		return apply_filters( 'cl_payment_customer_id', $this->customer_id, $this->ID, $this );
	}
	...
	private function get_user_id() {
		return apply_filters( 'cl_payment_user_id', $this->user_id, $this->ID, $this );
	}
	...
	private function get_email() {
		return apply_filters( 'cl_payment_user_email', $this->email, $this->ID, $this );
	}
	...
	private function get_user_info() {
		return apply_filters( 'cl_payment_meta_user_info', $this->user_info, $this->ID, $this );
	}
	...
	private function get_address() {
		return apply_filters( 'cl_payment_address', $this->address, $this->ID, $this );
	}
	...
	private function get_key() {
		return apply_filters( 'cl_payment_key', $this->key, $this->ID, $this );
	}
	...
	private function get_number() {
		 return apply_filters( 'cl_payment_number', $this->number, $this->ID, $this );
	}

	/**
	 * Retrieve listing on payment
	 *
	 * @since  2.5.1
	 * @return array Payment listing
	 */
	private function get_listing() {
		return apply_filters( 'cl_payment_meta_listing', $this->listing, $this->ID, $this );
	}

	/**
	 * Retrieve unlimited file listing status
	 *
	 * @since  2.5.1
	 * @return bool Is unlimited
	 */
	private function get_unlimited() {
		return apply_filters( 'cl_payment_unlimited_listing', $this->unlimited, $this->ID, $this );
	}
	 ...
	 * @since 2.8.4
	 *
	 * @return Customer The customer object of the existing customer or new customer.
	 */
	private function maybe_create_customer() {
		$customer = new \stdClass();

		if ( did_action( 'cl_pre_process_purchase' ) && is_user_logged_in() ) {

			$customer = new Customer( get_current_user_id(), true );
