...
class CL_Stripe_API {
	...
	public function request( $object, $method ) {
		...
		// __('Unable to process request: Unmet Stripe payment gateway requirements. Please contact the website administrator.', 'essential-wp-real-estate')
		// );
		// }
		...
		if ( ! is_callable( array( $classname, $method ) ) ) {
			throw new CL_Stripe_Utils_Exceptions_Stripe_Object_Not_Found(
				sprintf(
					...
					esc_html__( 'Unable to call %1$s::%2$s', 'essential-wp-real-estate' ),
					$classname,
					$method
				)
			);
	...
	public function set_app_info() {
		Stripe::setAppInfo(
			'WordPress Property Listing Plugin - Stripe',
			CL_STRIPE_VERSION,
			esc_url( site_url() ),
