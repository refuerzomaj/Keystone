<?php
namespace  Essential\Restate\Front\CL_Logging;
...
class CL_Logging {
	...
	public function setup_log_file() {
		...
		$this->filename = wp_hash( home_url( '/' ) ) . '-cl-debug.log';
	...
	/**
	 * Registers the cl_log Post Type
	 *
	 * @since 1.3.1
	 * @return void
	 */
	public function register_post_type() {
		/* Logs post type */
		$log_args = array(
			'labels'              => array( 'name' => __( 'Logs', 'essential-wp-real-estate' ) ),
			'public'              => false,
		...
		register_post_type( 'cl_log', $log_args );
	}
	...
	public function register_taxonomy() {
		register_taxonomy( 'cl_log_type', 'cl_log', array( 'public' => false ) );
	}
	...
	public function log_types() {
		...
		return apply_filters( 'cl_log_types', $terms );
	}
	 ...
	 *
	 * @since 1.3.1
	 * @uses CL_Logging::get_connected_logs()
	...
	function insert_log( $log_data = array(), $log_meta = array() ) {
		...
		do_action( 'cl_pre_insert_log', $log_data, $log_meta );
		...
		if ( $log_data['log_type'] && $this->valid_type( $log_data['log_type'] ) ) {
			wp_set_object_terms( $log_id, $log_data['log_type'], 'cl_log_type', false );
		}

		// Set log meta, if any
		if ( $log_id && ! empty( $log_meta ) ) {
			foreach ( (array) $log_meta as $key => $meta ) {
				update_post_meta( $log_id, '_cl_log_' . sanitize_key( $key ), $meta );
			}
		}

		do_action( 'cl_post_insert_log', $log_id, $log_data, $log_meta );
	...
	public function update_log( $log_data = array(), $log_meta = array() ) {
		...
		do_action( 'cl_pre_update_log', $log_data, $log_meta );
		...
		do_action( 'cl_post_update_log', $log_id, $log_data, $log_meta );
	}
	...
	/**
	 * Return the location of the log file that CL_Logging will use.
	 *
	 * Note: Do not use this file to write to the logs, please use the `cl_debug_log` function to do so.
	 *
	 * @since 2.9.1
	 *
	 * @return string
