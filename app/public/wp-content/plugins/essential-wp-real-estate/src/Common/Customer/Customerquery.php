...
class Customerquery {
	...
	public function __construct( $query = '', $cl_db_customers = null ) {
		if ( $cl_db_customers ) {
			$this->cl_db_customers = $cl_db_customers;
		} else {
			$this->cl_db_customers = WPERECCP()->common->dbcustomer;
		}

		$this->table_name  = $this->cl_db_customers->table_name;
		$this->meta_type   = $this->cl_db_customers->meta_type;
		$this->primary_key = $this->cl_db_customers->primary_key;
		$this->date_key    = $this->cl_db_customers->date_key;
		$this->cache_group = $this->cl_db_customers->cache_group;
	...
	protected function parse_query() {
		 ...
		 * @since 2.8
		 */
		do_action_ref_array( 'cl_parse_customer_query', array( &$this ) );
	}
	...
	protected function get_items() {
		 ...
		 * @since 2.8
		 */
		do_action_ref_array( 'cl_pre_get_customers', array( &$this ) );

		// $args can include anything. Only use the args defined in the query_var_defaults to compute the key.
		$key = md5( serialize( wp_array_slice_assoc( $this->query_vars, array_keys( $this->query_var_defaults ) ) ) );

		$last_changed = $this->cl_db_customers->get_last_changed();
	...
	protected function set_found_items() {
		...
		if ( $this->query_vars['number'] && ! $this->query_vars['no_found_rows'] ) {
			 ...
			 */
			$found_items_query = apply_filters( 'cl_found_customers_query', 'SELECT FOUND_ROWS()', $this );
	...
	protected function get_allowed_orderby_keys() {
		 return array_keys( $this->cl_db_customers->get_columns() );
	}
}
