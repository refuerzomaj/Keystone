...
class Customer {
	...
	public function __get( $key ) {
		...
		if ( method_exists( $this, 'get_' . $key ) ) {
			...
			return new WP_Error( 'cl-customer-invalid-property', sprintf( __( 'Can\'t get property %s', 'essential-wp-real-estate' ), $key ) );
		}
	}
	...
	public function create( $data = array() ) {
		 ...
		 */
		do_action( 'cl_customer_pre_create', $args );
		 ...
		 * @param array $args Contains customer information such as payment ID, name, and email.
		 */
		do_action( 'cl_customer_post_create', $created, $args );
	...
	public function update( $data = array() ) {
		...
		do_action( 'cl_customer_pre_update', $this->id, $data );
		...
		do_action( 'cl_customer_post_update', $updated, $this->id, $data );
	...
	public function add_email( $email = '', $primary = false ) {
		...
		do_action( 'cl_customer_pre_add_email', $email, $this->id, $this );

		// Update is used to ensure duplicate emails are not added
		$ret = (bool) $this->add_meta( 'additional_email', $email );

		do_action( 'cl_customer_post_add_email', $email, $this->id, $this );
	...
	public function remove_email( $email = '' ) {
		...
		do_action( 'cl_customer_pre_remove_email', $email, $this->id, $this );

		$ret = (bool) $this->delete_meta( 'additional_email', $email );

		do_action( 'cl_customer_post_remove_email', $email, $this->id, $this );
	...
	public function set_primary_email( $new_primary_email = '' ) {
		...
		do_action( 'cl_customer_pre_set_primary_email', $new_primary_email, $this->id, $this );

		$existing = new Customer( $new_primary_email );
		...
		if ( $ret ) {
			...
			if ( $payment_ids ) {
				...
				foreach ( $payment_ids as $payment_id ) {
					...
					cl_update_payment_meta( $payment_id, 'email', $new_primary_email );
				}
			}
		}

		do_action( 'cl_customer_post_set_primary_email', $new_primary_email, $this->id, $this );
	...
	/*
	 * Get an array of Clpayment objects from the payment_ids attached to the customer
	 *
	 * @since  2.6
	 * @param  array|string  $status A single status as a string or an array of statuses
	 * @return array                 An array of Clpayment objects or an empty array
	 */
	public function get_payments( $status = array() ) {
		...
		foreach ( $payment_ids as $payment_id ) {
			...
			if ( empty( $status ) || ( is_array( $status ) && in_array( $payment->status, $status ) ) || $status == $payment->status ) {
				$payments[] = new Clpayment( $payment_id );
			}
		}
	...
	public function attach_payment( $payment_id = 0, $update_stats = true ) {
		...
		do_action( 'cl_customer_pre_attach_payment', $payment->ID, $this->id, $this );
		...
		do_action( 'cl_customer_post_attach_payment', $payment_added, $payment->ID, $this->id, $this );
	...
	public function remove_payment( $payment_id = 0, $update_stats = true ) {
		...
		do_action( 'cl_customer_pre_remove_payment', $payment->ID, $this->id );
		...
		do_action( 'cl_customer_post_remove_payment', $payment_removed, $payment->ID, $this->id, $this );
	...
	public function increase_purchase_count( $count = 1 ) {
		...
		do_action( 'cl_customer_pre_increase_purchase_count', $count, $this->id, $this );

		if ( $this->update( array( 'purchase_count' => $new_total ) ) ) {
			$this->purchase_count = $new_total;
		}

		do_action( 'cl_customer_post_increase_purchase_count', $this->purchase_count, $count, $this->id, $this );
	...
	public function decrease_purchase_count( $count = 1 ) {
		...
		do_action( 'cl_customer_pre_decrease_purchase_count', $count, $this->id, $this );

		if ( $this->update( array( 'purchase_count' => $new_total ) ) ) {
			$this->purchase_count = $new_total;
		}

		do_action( 'cl_customer_post_decrease_purchase_count', $this->purchase_count, $count, $this->id, $this );
	...
	public function increase_value( $value = 0.00 ) {
		$value     = floatval( apply_filters( 'cl_customer_increase_value', $value, $this ) );
		$new_value = floatval( $this->purchase_value ) + $value;

		do_action( 'cl_customer_pre_increase_value', $value, $this->id, $this );

		if ( $this->update( array( 'purchase_value' => $new_value ) ) ) {
			$this->purchase_value = $new_value;
		}

		do_action( 'cl_customer_post_increase_value', $this->purchase_value, $value, $this->id, $this );
	...
	public function decrease_value( $value = 0.00 ) {
		$value = apply_filters( 'cl_customer_decrease_value', $value, $this );
		...
		do_action( 'cl_customer_pre_decrease_value', $value, $this->id, $this );

		if ( $this->update( array( 'purchase_value' => $new_value ) ) ) {
			$this->purchase_value = $new_value;
		}

		do_action( 'cl_customer_post_decrease_value', $this->purchase_value, $value, $this->id, $this );
	...
	public function add_note( $note = '' ) {
		...
		$new_note    = apply_filters( 'cl_customer_add_note_string', $note_string );
		$notes      .= "\n\n" . $new_note;

		do_action( 'cl_customer_pre_add_note', $new_note, $this->id, $this );
		...
		do_action( 'cl_customer_post_add_note', $this->notes, $new_note, $this->id, $this );
	...
	public function get_meta( $meta_key = '', $single = true ) {
		return WPERECCP()->common->customermeta->get_meta( $this->id, $meta_key, $single );
	}
	...
	public function add_meta( $meta_key, $meta_value, $unique = false ) {
		return WPERECCP()->common->customermeta->add_meta( $this->id, $meta_key, $meta_value, $unique );
	}
	...
	public function update_meta( $meta_key, $meta_value, $prev_value = '' ) {
		return WPERECCP()->common->customermeta->update_meta( $this->id, $meta_key, $meta_value, $prev_value );
	}
	...
	public function delete_meta( $meta_key = '', $meta_value = '' ) {
		return WPERECCP()->common->customermeta->delete_meta( $this->id, $meta_key, $meta_value );
	}
			...
			switch ( $type ) {
				...
				case '%s':
					if ( 'email' == $key ) {
						...
						$data[ $key ] = cl_sanitization( $data[ $key ] );
					}
					break;
