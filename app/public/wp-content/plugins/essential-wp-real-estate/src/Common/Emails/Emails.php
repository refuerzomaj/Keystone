...
class Emails {
	...
	public function get_templates() {
		$templates = array(
			'default' => __( 'Default Template', 'essential-wp-real-estate' ),
			'none'    => __( 'No template, plain text only', 'essential-wp-real-estate' ),
		);
	...
	public function send( $to, $subject, $message, $attachments = '' ) {
		...
		if ( ! did_action( 'init' ) && ! did_action( 'admin_init' ) ) {
			_doing_it_wrong( __FUNCTION__, __( 'You cannot send email with Emails until init/admin_init has been reached', 'essential-wp-real-estate' ), null );
			return false;
		}
		/**
		...
		if ( ! $sent && true === $log_errors ) {
			...
			$log_message = sprintf(
				__( "Email from Property Listing Plugin failed to send.\nSend time: %1\$s\nTo: %2\$s\nSubject: %3\$s\n\n", 'essential-wp-real-estate' ),
