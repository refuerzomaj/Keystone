...
<div class="column col-<?php echo esc_attr( $columns . ' ' . $sortable ); ?>">
	...
	if ( is_array( $meta_clone_val['meta_val'] ) ) {
		foreach ( $meta_clone_val['meta_val'] as $val ) {
			...
			?>
			<label for="cl-meta-select-val"><?php echo esc_html( $cl_meta_field['name'] ); ?></label>
			<select name="<?php echo esc_attr( $meta_clone_val['cl_name'] ); ?>">
				<?php
		...
		<select name="<?php echo esc_attr( $meta_clone_val['cl_name'] ); ?>">
			<?php
			foreach ( $options as $option ) {
				if ( $option == $meta_clone_val['meta_val'] ) {
					echo '<option selected>' . esc_html( $option ) . '</option>';
				} else {
					echo '<option>' . esc_html( $option ) . '</option>';
				}
