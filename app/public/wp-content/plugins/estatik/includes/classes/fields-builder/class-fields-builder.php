...
class Es_Fields_Builder extends Es_Fields_Builder_Item {
	...
	public static function prepare_item_data( $section_data ) {
		$valid_cols = array( 'id', 'label', 'frontend_form_name', 'frontend_machine_name', 'type', 'values', 'options', 'machine_name', 'section_machine_name', 'tab_machine_name', 'entity_name', 'mandatory', 'search_support', 'mls_import_support', 'is_visible', 'is_visible_for', 'order', 'address_component' );
	...
	public static function save_field( $field_data ) {
		...
		$exists = ! empty( $field_data['machine_name'] ) ? static::exists( $field_data['machine_name'] ) : false;
