( function( $ ) {
    ...
    function initSearchDropdownLabels( $search_container ) {
        ...
        $( $search_container ).find( '.js-es-search-nav__single-item' ).each( function() {
            ...
            if ( is_range_mode ) {
                ...
                var max_value_label = $max_field.length && $max_field.prop( 'tagName' ).toLowerCase() === 'select' ? $max_field.find( 'option:selected' ).html() : min_value;
    ...
    function initSelect2() {
        $( '.es-search select' ).each( function() {
    ...
    $( function() {
        ...
        $( '.js-es-search__collapse-link' ).click( function() {
            $( this ).closest( '.es-search' ).find( '.es-search-nav' ).toggleClass( 'es-search-nav--show' );
